...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        In the code below, implement an GRU cell using @inputs
        ...
        with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
            ...
            W_z = tf.get_variable('W_z', shape=(self.state_size, self.state_size), initializer=initializer)
            U_z = tf.get_variable('U_z', shape=(self.input_size, self.state_size), initializer=initializer)
            b_z = tf.get_variable('b_z', shape=(self.state_size,), dtype=tf.float32, initializer=tf.zeros_initializer())
            z_t = tf.nn.sigmoid(tf.matmul(inputs, U_z) + tf.matmul(state, W_z) + b_z)

            W_r = tf.get_variable('W_r', shape=(self.state_size, self.state_size), initializer=initializer)
            U_r = tf.get_variable('U_r', shape=(self.input_size, self.state_size), initializer=initializer)
            b_r = tf.get_variable('b_r', shape=(self.state_size,), dtype=tf.float32, initializer=tf.zeros_initializer())
            r_t = tf.nn.sigmoid(tf.matmul(inputs, U_r) + tf.matmul(state, W_r) + b_r)

            W_o = tf.get_variable('W_o', shape=(self.state_size, self.state_size), initializer=initializer)
            U_o = tf.get_variable('U_o', shape=(self.input_size, self.state_size), initializer=initializer)
            b_o = tf.get_variable('b_o', shape=(self.state_size,), dtype=tf.float32, initializer=tf.zeros_initializer())
