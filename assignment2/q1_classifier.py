...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        """
        # YOUR CODE HERE
        with tf.variable_scope('simple_model', reuse=tf.AUTO_REUSE) as scope:
            w = tf.get_variable(name='weights', shape=(self.config.n_features, self.config.n_classes),
                                initializer=tf.truncated_normal_initializer(stddev=0.1))
            b = tf.get_variable(name='biases', shape=(self.config.n_classes,), initializer=tf.zeros_initializer())
